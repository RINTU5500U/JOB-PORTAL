...
module.exports = {
    ...
    findAllAppliedJobByMe : async (req, res) => {
        try {
    ...
    specificAppliedJobByMyPost : async (req, res) => {
        try {
            let {userId,jobId} = req.params
            let findJob = await jobApplyModel.findOne({userId: userId, jobId: jobId, isDeleted: false}).populate('userId').populate('jobId')
            if (!findJob) {
    ...
    allAppliedJobByMyPost : async (req, res) => {
        try {
